...
- **Un `T0` ne se confirme qu'après le second rideau** (`D-157`, arbitrage du
  responsable). Une quatrième condition dure s'ajoute aux trois de `D-052` :
  ce que le praticien a assigné **après la validation de la première synthèse**
  doit être rendu. Les trois autres conditions et la table `RIDEAU_T0` sont
  inchangées.
...
- **Neuf dossiers réels deviennent non confirmables** tant qu'un second rideau
  n'est pas assigné puis rendu. Au 2026-09-08 : 14 dossiers portent une synthèse
  validée, 4 ont déjà leur `T0`, et des 10 restants **2 seulement** ont des
  assignations postérieures — aucune rendue. C'est la demande, chiffrée avant
  d'être écrite.
