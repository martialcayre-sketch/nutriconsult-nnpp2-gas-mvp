 ...
 * HUIT colonnes hors de portée, et le motif de chacune (`createdAt` compris,
 * posé par la base et jamais touché) :
 ...
 *  - `cycleId` : celle dont l'écriture violerait l'index unique partiel
 *    `assessment_episodes_mesure_cycle_unique_idx` — `idPatient` et `milestone`
 *    entrent aussi dans les index partiels, mais ils sont déjà l'identité de la
 *    ligne, donc hors de portée à ce titre ;
